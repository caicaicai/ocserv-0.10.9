...
static int get_sup_config(struct cfg_st *cfg, client_entry_st *entry,
			  SecAuthSessionReplyMsg *msg, void *pool)
{
	syslog(LOG_DEBUG, "Config::........radius get_sup_config running....");
	...
	syslog(LOG_DEBUG, "Config::........Loading sub configuration.");

	if (cfg->per_group_dir != NULL && pctx->groupname != NULL) {
		snprintf(file, sizeof(file), "%s/%s", cfg->per_group_dir,
			 pctx->groupname);

		syslog(LOG_DEBUG, "Config::Loading per_group_dir: '%s%s'.", cfg->per_group_dir,
			 pctx->groupname);
	...
	syslog(LOG_DEBUG, "Config::........Loading sub configuration done!");
